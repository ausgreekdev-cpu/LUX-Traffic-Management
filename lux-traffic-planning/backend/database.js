...
function seedIfEmpty() {
  ...
  const insertTmp = db.prepare(
    `INSERT INTO tmps (id, tmp_number, project_name, request_date, client_name, location, date_of_works, details, assigned_to, priority, status, custom_fields, last_updated) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`
  );
  ...
  const mockData = [
    [
      'mock-1',
      'TMP-2024-001',
      'Main Street Utility Works',
      d(-14),
      'City Water Authority',
      '123 Main Street, Sydney CBD',
      d(5),
      'Installation of new water main requiring lane closures.',
      'Sarah Chen',
      'high',
      'new',
      '{}',
    ...
    [
      ...
      d(-21),
      'Transport NSW',
      'Warringah Expressway, North Sydney',
      d(12),
      'Routine structural inspection of bridge #4.',
      'Michael Torres',
      'medium',
      'in-progress',
      '{}',
    ...
    [
      ...
      d(18),
      'Asphalt resurfacing. Night works only.',
      'Emily Watson',
      'high',
      'permits-lga',
      '{}',
  ...
  ['new', 'in-progress', 'permits-lga', 'approvals', 'completed'].forEach((s, i, a) => {
    if (i < a.length - 1)
      defaultSettings.tableColumns[s] = [
...
function getAllTmps() {
  ...
  return rows.map((r) => ({
    id: r.id,
    tmpNumber: r.tmp_number,
    projectName: r.project_name,
    requestDate: r.request_date,
    clientName: r.client_name,
    location: r.location,
    dateOfWorks: r.date_of_works,
    details: r.details,
...
function createTmp(data) {
  ...
  const tn =
    data.tmpNumber ||
    'TMP-' + new Date().getFullYear() + '-' + String(Math.floor(Math.random() * 999)).padStart(3, '0');
...
function updateTmp(id, data) {
  ...
  run(
    `UPDATE tmps SET tmp_number = ?, project_name = ?, request_date = ?, client_name = ?, location = ?, date_of_works = ?, details = ?, assigned_to = ?, priority = ?, status = ?, custom_fields = ?, last_updated = ? WHERE id = ?`,
...
function addAuditLog(action, tmpId, tmpNumber, username, details, timestamp) {
  ...
  run(
    'INSERT INTO audit_log (id, action, tmp_id, tmp_number, username, details, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)',
