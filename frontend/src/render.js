...
export function renderSidebar(state, getEnabledMenuItems, getCountByStatus, canManageSettings, escHtml) {
  ...
  if (canManageSettings()) items = [...items, { id: 'settings', label: 'Admin Settings', icon: '\u2699\ufe0f', enabled: true, badge: false }];
...
export function renderDashboard(state, getActiveStatuses, getCountByStatus, getUpcomingWorks, getStatusStyle, getStatusLabel, getPriorityStyle, getPriorityLabel, renderStatusWithScheduled, formatDate, escHtml) {
  const DEFAULT_DASHBOARD_WIDGETS = { statActiveTMPs: true, statPendingApprovals: true, statUpcomingWorks: true, statHighPriority: true, chartStatusDistribution: true };
  const DEFAULT_PRIORITIES = [{ id: 'low', label: 'Low', bg: '#d1fae5', color: '#065f46' }, { id: 'medium', label: 'Medium', bg: '#fef3c7', color: '#92400e' }, { id: 'high', label: 'High', bg: '#fee2e2', color: '#991b1b' }];
  ...
  const dist = {};
  getActiveStatuses().forEach(s => { dist[s.id] = getCountByStatus(s.id); });
  const mx = Math.max(...Object.values(dist), 1);
  const bars = getActiveStatuses().map(s => `<div class="chart-bar-group"><div class="chart-bar" style="height:${Math.max((dist[s.id] || 0) / mx * 100, 4)}%;background:${s.color}"><span class="bar-count">${dist[s.id] || 0}</span></div><span class="chart-bar-label">${escHtml(s.label)}</span></div>`).join('');
  const recent = [...state.tmps].sort((a, b) => new Date(b.lastUpdated) - new Date(a.lastUpdated)).slice(0, 5);
  const rows = recent.length ? recent.map(t => `<tr><td><strong>${escHtml(t.tmpNumber)}</strong></td><td>${escHtml(t.projectName)}</td><td>${escHtml(t.clientName)}</td><td><span class="status-badge" style="${getPriorityStyle(t.priority)}">${escHtml(getPriorityLabel(t.priority))}</span></td><td>${renderStatusWithScheduled(t)}</td><td>${formatDate(t.lastUpdated)}</td></tr>`).join('') : '<tr><td colspan="6" class="empty-state"><p>No records yet.</p></td></tr>';
  let stats = '';
  if (w.statActiveTMPs !== false) stats += `<div class="stat-card"><div class="stat-label">Active TMPs</div><div class="stat-value">${ta}</div></div>`;
  if (w.statPendingApprovals !== false) stats += `<div class="stat-card"><div class="stat-label">Pending Approvals</div><div class="stat-value">${pa}</div></div>`;
  if (w.statUpcomingWorks !== false) stats += `<div class="stat-card"><div class="stat-label">Upcoming Works (7d)</div><div class="stat-value">${up.length}</div></div>`;
  if (w.statHighPriority !== false) stats += `<div class="stat-card"><div class="stat-label">High Priority</div><div class="stat-value">${hp}</div></div>`;
  const chart = (w.chartStatusDistribution !== false) ? `<div class="chart-container"><h3>TMP Status Distribution</h3><div class="chart-bars">${bars}</div></div>` : '';
...
export function renderTable(status, state, getTmpsByStatus, getStatusLabel, getEnabledColumns, canEdit, canDelete, canAdvanceStatus, getActiveStatuses, getPriorityStyle, getPriorityLabel, formatDate, escHtml, getDensityClass) {
  ...
  const rows = items.length ? items.map(t => {
    const cells = cols.map(c => {
      if (c.key === 'actions') {
        const ed = canEdit() ? `<button class="btn btn-sm btn-outline" data-action="edit" data-id="${t.id}">\u270f\ufe0f</button>` : '';
        const del = canDelete() ? `<button class="btn btn-sm btn-danger" data-action="delete" data-id="${t.id}">\ud83d\uddd1</button>` : '';
        let adv = '';
      ...
      let val = t[c.key] || '\u2014';
      if (c.key === 'priority') val = `<span class="status-badge" style="${getPriorityStyle(t.priority)}">${escHtml(getPriorityLabel(t.priority))}</span>`;
  ...
  const empty = !items.length ? `<div class="empty-state"><div class="empty-icon">\ud83d\udccb</div><p>No TMPs in <strong>${escHtml(sl)}</strong>.</p></div>` : '';
...
export function renderCalendar(state, todayStr, daysInMonth, monthName, getDayOfWeek, escHtml) {
  ...
  const dh = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => `<div class="calendar-day-header">${d}</div>`).join('');
  const cells = [];
  for (let i = 0; i < fd; i++) cells.push(`<div class="calendar-day other-month"><span class="day-number">${pmd - fd + 1 + i}</span></div>`);
  const ebd = {};
  ...
  for (let d = 1; d <= tdy; d++) {
    ...
    const el = sh.map(e => `<div class="cal-event" style="background:${(state.settings.priorities || DEFAULT_PRIORITIES).find(p => p.id === e.priority)?.color || 'var(--primary-light)'}" data-action="view" data-id="${e.id}">${escHtml(e.tmpNumber)}</div>`).join('');
    cells.push(`<div class="calendar-day ${ds === td ? 'today' : ''}"><div class="day-number">${d}</div>${el}${mr > 0 ? `<div class="cal-more" style="color:var(--primary)">+${mr} more</div>` : ''}</div>`);
  }
  const tc = fd + tdy;
  const rem = (7 - (tc % 7)) % 7;
  for (let i = 1; i <= rem; i++) cells.push(`<div class="calendar-day other-month"><span class="day-number">${i}</span></div>`);
...
export function renderSettings(state, settingsTab, SETTINGS_TABS, DEFAULT_STATUSES, DEFAULT_PRIORITIES, DEFAULT_KEYBOARD_SHORTCUTS, DEFAULT_USERS, DEFAULT_DASHBOARD_WIDGETS, ROLES, getActiveStatuses, getStatusLabel, getStatusStyle, escHtml) {
  const th = SETTINGS_TABS.map(t => `<button class="settings-tab ${settingsTab === t.id ? 'active' : ''}" data-stab="${t.id}">${escHtml(t.label)}</button>`).join('');
...
function renderMenu(state, escHtml) {
  ...
  return `<p class="settings-hint">Toggle visibility, rename, reorder (drag \u2630 or use arrows), and delete menu items. Built-in items use their view ID for routing.</p>
    ...
    <div id="menuSortable">${items.map((m, i) => `
...
function renderContent(state, escHtml) {
  const c = state.settings.content;
  const views = Object.keys(c.viewTitles || {}).map(k => `<div class="setting-row"><div class="setting-label">${escHtml(k)} Title</div><div class="setting-actions"><input class="s-inline s-content-title" data-view="${k}" value="${escHtml(c.viewTitles[k])}" style="width:280px"></div></div>`).join('');
...
function renderTheme(state, escHtml) {
  ...
  const systemColors = [
    { key: 'primary', label: 'Primary' }, { key: 'primaryLight', label: 'Primary Light' }, { key: 'primaryDark', label: 'Primary Dark' },
    { key: 'bg', label: 'Page BG' }, { key: 'surface', label: 'Card BG' },
    { key: 'text', label: 'Text' }, { key: 'textSecondary', label: 'Text Secondary' }, { key: 'border', label: 'Border' },
    { key: 'success', label: 'Success' }, { key: 'warning', label: 'Warning' }, { key: 'danger', label: 'Danger' },
    { key: 'purple', label: 'Purple' }, { key: 'cyan', label: 'Cyan' },
  ];
  const sidebarColors = [
    { key: 'sidebarBg', label: 'Sidebar BG' }, { key: 'sidebarText', label: 'Sidebar Text' },
    { key: 'sidebarNavText', label: 'Nav Text' }, { key: 'sidebarHoverBg', label: 'Hover BG' },
    { key: 'sidebarActiveBg', label: 'Active BG' }, { key: 'sidebarHeaderBorder', label: 'Header Border' },
  ];
  const colorRow = (c) => `<div class="setting-row"><div class="setting-label">${c.label}</div><div class="setting-actions"><div class="color-input-group"><input type="color" class="s-theme-color" data-key="${c.key}" value="${t[c.key] || ''}"><input type="text" class="s-theme-hex" data-key="${c.key}" value="${t[c.key] || ''}" style="width:120px"></div></div></div>`;
  const sidebarBgImgPreview = t.sidebarBgImg ? `<div style="margin-top:8px"><img src="${escHtml(t.sidebarBgImg)}" style="max-height:60px;width:100%;object-fit:cover;border:1px solid var(--border);border-radius:4px"><br><button class="btn btn-xs btn-outline" data-action="remove-sidebar-bg" style="margin-top:4px">Remove Image</button></div>` : '<span style="color:var(--text-secondary);font-size:.85rem">No background image</span>';
  const logoPreview = t.logo ? `<div><img src="${escHtml(t.logo)}" style="max-height:60px;border:1px solid var(--border);border-radius:4px;padding:4px;background:#fff"><br><button class="btn btn-xs btn-outline" data-action="remove-logo" style="margin-top:4px">Remove Logo</button></div>` : '<span style="color:var(--text-secondary);font-size:.85rem">No logo</span>';
  const bannerPreview = t.banner ? `<div><img src="${escHtml(t.banner)}" style="max-height:80px;width:100%;object-fit:cover;border:1px solid var(--border);border-radius:4px"><br><button class="btn btn-xs btn-outline" data-action="remove-banner" style="margin-top:4px">Remove Banner</button></div>` : '<span style="color:var(--text-secondary);font-size:.85rem">No banner</span>';
...
function renderColumns(state, DEFAULT_STATUSES, escHtml) {
  ...
  ['new', 'in-progress', 'permits-lga', 'approvals', 'completed'].forEach(s => {
    DEFAULT_TABLE_COLUMNS[s] = [
      { key: 'tmpNumber', label: 'TMP#', enabled: true }, { key: 'projectName', label: 'Project', enabled: true },
      { key: 'clientName', label: 'Client', enabled: true }, { key: 'location', label: 'Location', enabled: true },
      { key: 'dateOfWorks', label: 'Date of Works', enabled: true }, { key: 'assignedTo', label: 'Assigned To', enabled: true },
      { key: 'priority', label: 'Priority', enabled: true }, { key: 'actions', label: 'Actions', enabled: true },
    ];
  });
  const cur = state.settings._colStatus || 'new';
  const sel = (state.settings.statuses || DEFAULT_STATUSES).map(s => `<option value="${s.id}" ${s.id === cur ? 'selected' : ''}>${escHtml(s.label)}</option>`).join('');
...
function renderCustomFields(state, escHtml) {
  ...
  const list = fields.length ? fields.map((f, i) => `
    ...
    </div>`).join('') : '<p style="color:var(--text-secondary);text-align:center;padding:20px">No custom fields yet.</p>';

  const ef = state.settings._editingCF !== undefined ? (state.settings.customFields[state.settings._editingCF] || { label: '', type: 'text', required: false, options: '', gridClass: '' }) : { label: '', type: 'text', required: false, options: '', gridClass: '' };
...
function renderUsers(state, DEFAULT_USERS, ROLES, escHtml) {
  ...
  const ef = state.settings._editingUser !== undefined ? (state.settings.users[state.settings._editingUser] || { username: '', password: '', role: 'viewer' }) : { username: '', password: '', role: 'viewer' };
  const editForm = state.settings._editingUser !== undefined ? `<div style="background:var(--bg);border-radius:var(--radius);padding:20px;margin-bottom:16px;border:1px solid var(--border)">
    ...
    <div class="form-grid" style="max-width:500px">
      ...
      <div class="form-group"><label>Role</label><select id="eu_role">${Object.keys(ROLES).map(r => '<option value="' + r + '" ' + (ef.role === r ? 'selected' : '') + '>' + ROLES[r].label + '</option>').join('')}</select></div>
    </div>
    <div style="margin-top:12px;display:flex;gap:8px">
...
function renderTransitions(state, ROLES, getActiveStatuses, getStatusStyle, escHtml) {
  ...
  return `<p class="settings-hint">Control which roles can advance TMPs from each status. Check the roles allowed to advance from each status.</p>
    ${statuses.map((s, i) => {
      ...
      const checks = allRoles.map(r => '<label style="display:inline-flex;align-items:center;gap:4px;margin-right:12px;font-size:.85rem;cursor:pointer"><input type="checkbox" class="s-transition-role" data-status="' + s.id + '" data-role="' + r + '" ' + (rule.canAdvanceRoles.includes(r) ? 'checked' : '') + '> ' + ROLES[r].label + '</label>').join('');
...
function renderShortcuts(state, DEFAULT_KEYBOARD_SHORTCUTS, escHtml) {
  ...
  return `<p class="settings-hint">Configure keyboard shortcuts. Use standard key notation (e.g. ctrl+n, ctrl+shift+e).</p>
    ${Object.keys(DEFAULT_KEYBOARD_SHORTCUTS).map(k => {
      const labels = { newRequest: 'New Request', globalSearch: 'Global Search', exportJSON: 'Export JSON', dashboard: 'Navigate: Dashboard', calendar: 'Navigate: Calendar', adminSettings: 'Admin Settings', help: 'Help / Shortcuts' };
...
function renderAutomations(state, getActiveStatuses, getStatusLabel, escHtml) {
  ...
  const statuses = getActiveStatuses();
  const statusOpts = statuses.map((s, i) => (i < statuses.length - 1) ? `<option value="${s.id}">${escHtml(s.label)}</option>` : '').join('');
  const ef = state.settings._editingAuto !== undefined ? (state.settings.automations[state.settings._editingAuto] || { fromStatus: '', toStatus: '', delayDays: 1, enabled: true }) : { fromStatus: '', toStatus: '', delayDays: 1, enabled: true };
  const editForm = state.settings._editingAuto !== undefined ? `<div style="background:var(--bg);border-radius:var(--radius);padding:20px;margin-bottom:16px;border:1px solid var(--border)">
  ...
  return `<p class="settings-hint">Auto-advance rules: when a TMP reaches a status, automatically advance it after N days.</p>
    ...
    ${a.length ? a.map((r, i) => `
      ...
      </div>`).join('') : '<p style="text-align:center;padding:20px;color:var(--text-secondary)">No automation rules yet. Add one to auto-advance TMPs after a delay.</p>'}`;
}

function renderClients(state, escHtml) {
  const clients = state.settings.clients || [];
  const ef = state.settings._editingClient !== undefined ? (state.settings.clients[state.settings._editingClient] || { name: '', type: 'government', contact: '', phone: '' }) : { name: '', type: 'government', contact: '', phone: '' };
  const editForm = state.settings._editingClient !== undefined ? `<div style="background:var(--bg);border-radius:var(--radius);padding:20px;margin-bottom:16px;border:1px solid var(--border)">
...
function renderAuditLog(state, escHtml) {
  ...
  const rows = logs.length ? logs.slice().reverse().slice(0, 200).map(l => `
    <tr>
    ...
    </tr>`).join('') : '<tr><td colspan="5" class="empty-state"><p>No audit log entries yet.</p></td></tr>';
