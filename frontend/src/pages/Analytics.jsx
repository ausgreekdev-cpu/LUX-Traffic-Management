            ...
            <div className="card p-4">
              ...
              {approval.metrics?.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead className="border-b border-gray-200 dark:border-gray-700"><tr><th className="table-th">Authority</th><th className="table-th">Type</th><th className="table-th">Avg Days</th><th className="table-th">Min</th><th className="table-th">Max</th><th className="table-th">Sample</th></tr></thead>
                  <tbody className="divide-y dark:divide-gray-700">{approval.metrics.map(m => (
                    <tr key={m.authority_id} className="hover:bg-gray-50 dark:hover:bg-gray-700"><td className="table-td font-medium">{m.authority_name}</td><td className="table-td text-gray-500">{m.authority_type?.toUpperCase()}</td><td className={`table-td ${m.avg_approval_days < 14 ? 'text-green-600' : m.avg_approval_days < 20 ? 'text-lux-600' : 'text-red-600'}`}>{m.avg_approval_days}d</td><td className="table-td">{m.min_approval_days}d</td><td className="table-td">{m.max_approval_days}d</td><td className="table-td">{m.sample_count}</td></tr>
                  ))}</tbody>
                </table>
                </div>
          ...
          {(tab === 'overview' || tab === 'financial') && financial && (
            <div className="card p-4">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-center">
                <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg"><div className="text-xl font-bold text-green-500">${financial.fees?.total_paid?.toFixed(0) || 0}</div><div className="text-xs text-gray-500">Paid</div></div>
                <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg"><div className="text-xl font-bold text-lux-500">${financial.fees?.total_pending?.toFixed(0) || 0}</div><div className="text-xs text-gray-500">Pending</div></div>
                <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg"><div className="text-xl font-bold text-blue-500">${financial.time?.billable_cost?.toFixed(0) || 0}</div><div className="text-xs text-gray-500">Billable Labor</div></div>
                <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg"><div className="text-xl font-bold text-purple-500">${financial.fees?.bonds_held?.toFixed(0) || 0}</div><div className="text-xs text-gray-500">Bonds Held</div></div>
              </div>
