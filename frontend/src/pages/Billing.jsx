...
export default function Billing() {
  ...
  const checkout = async (tier) => {
    const priceMap = { starter: import.meta.env.VITE_STRIPE_PRICE_STARTER, pro: import.meta.env.VITE_STRIPE_PRICE_PRO, agency: import.meta.env.VITE_STRIPE_PRICE_AGENCY };
    const res = await fetch('/api/billing/checkout', { method:'POST', headers:{'Content-Type':'application/json', Authorization:`Bearer ${localStorage.getItem('token')}`}, body: JSON.stringify({ priceId: priceMap[tier] })});
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <p className="text-gray-500">Current: <span className="font-semibold">{ent?.tenant?.plan || '—'}</span> {ent?.tenant?.status ? `(${ent.tenant.status})` : ''}</p>
      <div className="grid md:grid-cols-3 gap-6 mt-6">
        {TIERS.map(t=>(
          <div key={t.id} className={`border rounded-xl p-6 ${t.highlight?'border-amber-400 shadow-lg scale-[1.02]':''}`}>
            <h2 className="text-xl font-bold">{t.name}</h2>
            <p className="text-2xl font-extrabold mt-2">{t.price} <span className="text-sm font-normal text-gray-500">{t.annual}</span></p>
